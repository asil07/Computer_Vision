...
def classify_batch(model, batchROIs, batchLocs, labels, minProb=0.5, top=10, dims=(224,224)):
    ...
    for i in range(0, len(P)):
        for (_, label, prob) in P[i]:
            # Filter outs weak detection
            if prob > minProb:
                # grabs coordinates of sliding window
                (pX, pY) = batchLocs[i]
                box = (pX, pY, pX + dims[0], pY + dims[1])

                # grabs the list of predictions for the label and adds the bounding box + probability to the list
                L = labels.get(label, [])
                L.append((box, prob))
                labels[label] = L
    return labels
